CREATE OR REPLACE FUNCTION search_movies_hybrid(
  ...
  filter_min_duration INT DEFAULT NULL,
...
AS $$
  ...
  WHERE
    ...
    AND (filter_min_duration IS NULL OR movies.runtime_minutes >= filter_min_duration)
    AND (filter_max_duration IS NULL OR movies.runtime_minutes <= filter_max_duration)
